...
// Default tag colors that match the TagComponent colors
...
const DEFAULT_CONTACT_TAGS = [
    ...
    { name: "peer", color: "ROSE" }
];

const ContactTagCreator = ({ getSelectedTags, accountId, defaultTags }: Props) => {
    ...
    useEffect(() => {
        if (accountId) {
            const fetchData = async () => {
                ...
                if (response) {
                    let existingTags = response.Tags || [];
                    
                    // Check if default tags exist, if not create them
                    const defaultTagsToCreate = [];
                    for (const defaultTag of DEFAULT_CONTACT_TAGS) {
                        if (!existingTags.some(tag => tag.name.toLowerCase() === defaultTag.name.toLowerCase())) {
                            defaultTagsToCreate.push({
                    ...
                    if (defaultTagsToCreate.length > 0) {
                        for (const tagData of defaultTagsToCreate) {
                            try {
    ...
    const handleDeleteTag = async (tagId: string) => {
        ...
        if (tagToDelete && DEFAULT_CONTACT_TAGS.some(dt => dt.name.toLowerCase() === tagToDelete.name.toLowerCase())) {
