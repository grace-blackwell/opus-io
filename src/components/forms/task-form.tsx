...
import { Briefcase, CheckIcon, ChevronsUpDownIcon } from "lucide-react";
import {
  Command,
  CommandEmpty,
  CommandGroup,
...
type Props = {
  laneId: string;
  accountId: string;
  getNewTask: (task: TaskWithTags) => void;
...
const TaskForm = ({
  ...
  const [project, setProject] = useState(
    defaultProjectId ||
      defaultData.task?.projectId ||
      defaultData.task?.Project?.id ||
      ""
  );
  ...
  useEffect(() => {
    if (defaultData.task) {
      ...
      else if (defaultData.task.Project?.id)
        setProject(defaultData.task.Project.id);
  ...
  const onSubmit = async (values: z.infer<typeof TaskFormSchema>) => {
    ...
    try {
      ...
      toast("Oops...", {
        description: "Something went wrong while saving task details.",
      });
    }
    setClose();
  };
                ...
                >
                  {contact ? (
                    <div className="flex items-center gap-2">
                      ...
                      <span>
                        {contactList.find((c) => c.id === contact)?.contactName}
                      </span>
                    </div>
                  ) : (
                    "Select Contact..."
