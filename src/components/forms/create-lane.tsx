...
import {getKanbanDetails, saveActivityLogNotification, upsertLane} from "@/lib/queries";
...
export const CreateLaneForm: React.FC<Props> = ({kanbanId, defaultData}: Props) => {
    ...
    const onSubmit = async (values: z.infer<typeof CreateLaneFormSchema>) => {
        ...
        try {
            const response = await upsertLane({
                ...values, id: defaultData?.id, kanbanId: kanbanId, order: defaultData?.order
            })

            const d = await getKanbanDetails(kanbanId)
            if (!d) return

            await saveActivityLogNotification(d.accountId, `Updated a lane: ${response.name}` )

            toast.success('Successfully saved lane details')
            router.refresh()
        } catch (error) {
            toast.error('Oops...', {description: 'Something went wrong while saving lane details.'})
        }
        setClose()
    }
