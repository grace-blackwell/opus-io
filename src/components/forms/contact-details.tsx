...
type ContactTagWithCustomColor = ContactTag & {
  colorHex?: string[];
};
...
const ContactDetails: React.FC<Props> = ({ accountData, data }) => {
  ...
  const [selectedTags, setSelectedTags] = useState<ContactTagWithCustomColor[]>(
    []
  );
    ...
    const loadTags = async () => {
      if (data?.id && accountData?.id) {
        try {
          ...
          const contactWithTags = await fetch(`/api/contacts/${data.id}`).then(
            (res) => res.json()
          );
          ...
          if (contactWithTags && contactWithTags.ContactTags) {
            ...
            const processedTags = contactWithTags.ContactTags.map(
              (contactTag: ContactTag): ContactTagWithCustomColor => {
                // Check if the color field contains a custom color
                if (contactTag.color.startsWith("CUSTOM:")) {
                  const colorHex = contactTag.color.split(":");
                  return {
                    ...contactTag,
                    colorHex,
                    color: "CUSTOM",
  ...
  async function onSubmit(values: z.infer<typeof FormSchema>) {
    try {
      ...
      toast.success(
        `Successfully ${isNewContact ? "created a new" : "updated"} contact`,
        {
  ...
  return (
    <Card className="w-full bg-base-100">
      <CardHeader className={"text-base-content"}>
        ...
        <CardDescription>Please enter contact details.</CardDescription>
      </CardHeader>
