    ...
    async function onSubmit(values: z.infer<typeof FormSchema>) {
        try {
            ...
            await saveActivityLogNotification(
                accountId, 
                `${projects?.id ? 'Updated' : 'Created new'} project: ${values.projectTitle}`
            );

            toast.success(`Successfully ${projects?.id ? 'updated' : 'created'} project`, {
                ...
                <Form {...form}>
                    <form
                        ...
                        <div className='flex md:flex-row gap-4'>
                            <FormField disabled={isLoading} control={form.control} name="projectTitle"
                                render={({ field }) => (
                                    <FormItem className="flex-1">
                                        <FormLabel>* Project Title</FormLabel>
                                        <FormControl>
                                            <Input style={customInputStyles}
                                                placeholder='Project Title' {...field} />
                                        </FormControl>
                                        <FormMessage />
                                    </FormItem>
                                )}></FormField>
                            ...
                            <FormField disabled={isLoading} control={form.control} name="projectId"
                                render={({ field }) => (
                                    <FormItem className="flex-1">
                                        ...
                                        <FormControl>
                                            <Input style={customInputStyles}
                                                placeholder='Project ID or Reference Number' {...field} />
                                        </FormControl>
                                        <FormMessage />
                                    </FormItem>
                                )}></FormField>
                        ...
                        <div className='flex md:flex-row gap-4'>
                            ...
                            <FormField disabled={isLoading} control={form.control} name="status"
                                render={({ field }) => (
                                    <FormItem className="flex-1">
                        ...
                        <div className='flex md:flex-row gap-4'>
                            <FormField disabled={isLoading} control={form.control} name="estimatedHours"
                                render={({ field }) => (
                                    <FormItem className="flex-1">
                                        <FormLabel>Estimated Hours</FormLabel>
                                        <FormControl>
                                            <Input style={customInputStyles} type="number" step="0.01"
                                                placeholder='Estimated Hours' {...field} />
                                        </FormControl>
                                        <FormMessage />
                                    </FormItem>
                                )}></FormField>

                            <FormField disabled={isLoading} control={form.control} name="estimatedCost"
                                render={({ field }) => (
                                    <FormItem className="flex-1">
                                        <FormLabel>Estimated Cost</FormLabel>
                                        <FormControl>
                                            <Input style={customInputStyles} type="number" step="0.01"
                                                placeholder='Estimated Cost' {...field} />
                                        </FormControl>
                                        <FormMessage />
                                    </FormItem>
                                )}></FormField>
                        ...
                        <div className='flex md:flex-row gap-4'>
                            <FormField disabled={isLoading} control={form.control} name="actualHours"
                                render={({ field }) => (
                                    <FormItem className="flex-1">
                                        <FormLabel>Actual Hours</FormLabel>
                                        <FormControl>
                                            <Input style={customInputStyles} type="number" step="0.01"
                                                placeholder='Actual Hours' {...field} />
                                        </FormControl>
                                        <FormMessage />
                                    </FormItem>
                                )}></FormField>

                            <FormField disabled={isLoading} control={form.control} name="actualCost"
                                render={({ field }) => (
                                    <FormItem className="flex-1">
                                        <FormLabel>Actual Cost</FormLabel>
                                        <FormControl>
                                            <Input style={customInputStyles} type="number" step="0.01"
                                                placeholder='Actual Cost' {...field} />
                                        </FormControl>
                                        <FormMessage />
                                    </FormItem>
                                )}></FormField>
                        ...
                        <div className="flex justify-end">
                            <Button type="submit" disabled={isLoading} className="mt-4">
                                {isLoading ? <Loading /> : projects?.id ? "Update Project" : "Create Project"}
                            </Button>
                        </div>
                    </form>
                </Form>
