...
// Custom components
import InvoicePdf from "@/app/(main)/account/[accountId]/invoice-generator/_components/invoice-pdf";
import {
  createOrUpdateInvoice,
  saveActivityLogNotification,
  convertToInvoiceWithRelations,
} from "@/lib/queries";
...
const InvoiceGenerator: React.FC<Props> = ({
  ...
  const [formattedInvoiceData, setFormattedInvoiceData] =
    useState<InvoicesWithAccountContactContractProject | null>(null);
  ...
  const salesTaxRateValue = form.watch("salesTaxRate");
  ...
  useEffect(() => {
    ...
    const salesTaxRate = parseFloat(salesTaxRateValue || "0");
    if (salesTaxRate) {
  ...
  const generatePreview = async () => {
    ...
    // Format the invoice data for the PDF
    convertToInvoiceWithRelations(previewData, accountId)
      .then((formattedData) => {
        setFormattedInvoiceData(formattedData);
        setPreviewMode(true);
      })
      .catch((error) => {
        console.error("Error formatting invoice data:", error);
        toast.error("Error preparing invoice preview");
      });
  };
  ...
  const sendInvoice = async () => {
    ...
    try {
      // Use the already formatted invoice data if available, otherwise format it
      const formattedInvoice =
        formattedInvoiceData ||
        (await convertToInvoiceWithRelations(invoiceData, accountId));

      // If we had to format it now, save it for future use
      if (!formattedInvoiceData) {
        setFormattedInvoiceData(formattedInvoice);
      }

      // Use the server action to send the email
      const result = await emailInvoiceToContact(formattedInvoice);
                ...
                <table className="w-full">
                  <thead>
                    <tr className="border-b">
                      <th className="text-left py-2 px-1 w-[50%]">
                        Description
                      </th>
        ...
        {invoiceData ? (
          <div className="space-y-6">
                ...
                <div className="w-full max-w-4xl">
                  ...
                  <div className="border p-4 rounded-md min-h-[600px] flex items-center justify-center">
                    <p className="text-muted-foreground">
                      PDF preview not available in browser. Please use the
            ...
            <div className="flex justify-end space-x-2">
              ...
              {formattedInvoiceData ? (
                <PDFDownloadLink
                  document={<InvoicePdf invoice={formattedInvoiceData} />}
