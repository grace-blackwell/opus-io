...
>[] = [
  ...
  },
  {
    ...
    cell: ({ row }) => {
      ...
      return <div className="flex items-center gap-4">{projectTitle}</div>;
    },
  },
  {
    accessorKey: "Contact",
    ...
    cell: ({ row }) => {
      ...
      return <div className="flex items-center gap-4">{ContactName}</div>;
    },
  },
  {
    ...
    cell: ({ row }) => {
      ...
      return (
        <div className="flex items-center gap-4">
          <span className="badge badge-soft badge-success badge-md">
            {formatted}
          </span>
        </div>
      );
  ...
  },
  {
    ...
    cell: ({ row }) => {
      ...
      return <div className="flex items-center gap-4">{paymentStatus}</div>;
    },
  },
  {
    id: "actions",
        ...
        <DropdownMenuContent align="end">
          <DropdownMenuItem
            ...
            onClick={() => {
              setOpen(
                <CustomModal
                  subheading="Enter the invoice details"
                ...
                async () => {
                  const invoice = await getInvoice(rowData?.id);
                  if (!invoice) return { invoice: undefined };

                  // Convert the invoice to the expected type by omitting the Contract property
                  const invoiceWithoutContract = { ...invoice } as {
                    Contract?: {
                      id: string;
                  ...
                  delete invoiceWithoutContract.Contract;
                  return {
                    invoice:
                      invoiceWithoutContract as InvoicesWithAccountContactContractProject,
                  };
                }
              );
            }}
            ...
            onClick={() => {
              if (!rowData.Contact?.contactEmail) {
                toast.error("Cannot Send Email", {
                  description:
                    "Contact email address not found. Please add an email address to the contact.",
                });
                return;
              }
              ...
              const subject = `Invoice #${rowData.invoiceNumber} from ${
...
Please find attached the invoice #${rowData.invoiceNumber} for ${
                rowData.Project?.projectTitle || "our services"
              }.
          ...
          <DropdownMenuItem
            ...
            onClick={async () => {
              ...
              toast.info("Sending Email...", {
                description: `Sending invoice #${rowData.invoiceNumber.toString()} to ${
                  rowData.Contact.contactEmail
                }.`,
              });
              ...
              try {
                ...
                if (result.success) {
                  toast.success("Email Sent", {
                    description: `Invoice #${rowData.invoiceNumber.toString()} has been sent to ${
                      rowData.Contact.contactEmail
                    }.`,
                  });
                } else {
