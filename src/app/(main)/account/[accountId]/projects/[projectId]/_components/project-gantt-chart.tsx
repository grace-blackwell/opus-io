...
export default function ProjectGanttChart({ projectId }: ProjectGanttChartProps) {
  ...
  const [taskMap, setTaskMap] = useState<Map<string, ProjectTask>>(new Map())
  ...
  useEffect(() => {
    const fetchTimeEntries = async () => {
      try {
        ...
        // Fetch time entries
        const timeEntriesResponse = await fetch(`/api/projects/${projectId}/time-entries`)
        ...
        const tasksWithEntries = Array.from(taskMap.values())
        ...
        if (timeEntriesData.length > 0) {
          ...
          const sortedByStart = [...timeEntriesData].sort((a, b) => 
          ...
          const ganttData: GanttChartData[] = tasksWithEntries.map(task => {
            const taskStartDates = task.entries.map(entry => startOfDay(new Date(entry.startTime)))
  ...
  if (timeEntries.length === 0 || !startDate || !endDate || projectTasks.length === 0) {
    return (
    ...
    <Card className={'bg-background border-none rounded-none shadow-none'}>
      <CardHeader>
        <div className="flex justify-between items-center">
          <div>
            <CardTitle>Project Timeline</CardTitle>
            <CardDescription>Gantt Chart showing {projectTasks.length} tasks with time entries</CardDescription>
          </div>
          ...
          <ResponsiveContainer width={"100%"} height={600}>
            ...
            >
              ...
              <XAxis
                ...
                axisLine={true}
                tickLine={true}
                ticks={Array.from({ length: totalDays + 1 }, (_, i) => i).filter(i => i % Math.ceil(totalDays / 10) === 0 || i === totalDays)}
              />
              <YAxis
                dataKey="task"
                type="category"
              ...
              <Bar
                ...
                shape={(props) => {
                  ...
                  const data = props.payload as GanttChartData
