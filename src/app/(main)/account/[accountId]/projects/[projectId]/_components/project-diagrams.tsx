...
const ProjectDiagrams = ({ diagrams, accountId, projectId }: Props) => {
    ...
    const handleCreateDiagram = async () => {
        try {
            ...
            const emptyDiagramXml = '<?xml version="1.0" encoding="UTF-8"?><mxfile host="app.diagrams.net" modified="2023-01-01T00:00:00.000Z" agent="Mozilla/5.0" etag="1234" version="21.2.8" type="device"><diagram id="diagram" name="Page-1"><mxGraphModel dx="1050" dy="629" grid="1" gridSize="10" guides="1" tooltips="1" connect="1" arrows="1" fold="1" page="1" pageScale="1" pageWidth="850" pageHeight="1100" math="0" shadow="0"><root><mxCell id="0"/><mxCell id="1" parent="0"/></root></mxGraphModel></diagram></mxfile>'
            ...
            const url = `https://app.diagrams.net/?mode=browser&title=New Diagram&saveAndExit=1&spin=1&libraries=1&saveTarget=self&target=self&modified=unsavedChanges&saveFormat=xml&format=xml&save=${encodeURIComponent(callbackUrl)}`
    ...
    const handleEditDiagram = (diagram: Diagram) => {
        ...
        const url = `https://app.diagrams.net/?mode=browser&title=${encodeURIComponent(diagram.name)}&url=${encodeURIComponent(callbackUrl)}&saveAndExit=1&spin=1&libraries=1&saveTarget=self&target=self&modified=unsavedChanges&saveFormat=xml&format=xml&save=${encodeURIComponent(callbackUrl)}`
        window.open(url, '_blank')
            ...
            {diagramsList.length === 0 ? (
                <div className="flex flex-col items-center justify-center p-12 text-center">
                    ...
                    <p className="mt-2 text-sm text-muted-foreground">
                        Create your first diagram to visualize your project structure or workflow.
                    </p>
                    ...
                    {diagramsList.map((diagram) => (
                        <Card key={diagram.id} className="overflow-hidden">
                            ...
                            <CardHeader className="p-4">
                                ...
                                <CardDescription>
                                    Last updated: {format(new Date(diagram.updatedAt), 'MMM dd, yyyy')}
                                </CardDescription>
                            </CardHeader>
...
const CreateDiagramForm = ({ accountId, projectId, onSuccess }: CreateDiagramFormProps) => {
    ...
    const handleSubmit = async (e: React.FormEvent) => {
        ...
        try {
            const newDiagram = await createDiagram(accountId, projectId, {
                name,
                content: '<?xml version="1.0" encoding="UTF-8"?><mxfile host="Electron" modified="2023-01-01T00:00:00.000Z" agent="Mozilla/5.0 (Windows NT 10.0; Win64; x64) AppleWebKit/537.36 (KHTML, like Gecko) draw.io/21.2.8 Chrome/112.0.5615.165 Electron/24.2.0 Safari/537.36" etag="1234" version="21.2.8" type="device"><diagram id="diagram" name="Page-1"><mxGraphModel dx="1050" dy="629" grid="1" gridSize="10" guides="1" tooltips="1" connect="1" arrows="1" fold="1" page="1" pageScale="1" pageWidth="850" pageHeight="1100" math="0" shadow="0"><root><mxCell id="0"/><mxCell id="1" parent="0"/></root></mxGraphModel></diagram></mxfile>',
            })
