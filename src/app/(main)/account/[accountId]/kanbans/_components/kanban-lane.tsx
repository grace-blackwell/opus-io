...
interface KanbanLaneProps {
  setAllTasks: Dispatch<SetStateAction<Array<TaskWithTags>>>;
  allTasks: Array<TaskWithTags>;
  tasks: Array<TaskWithTags>;
  kanbanId: string;
...
const KanbanLane: FC<KanbanLaneProps> = ({
  ...
  const laneAmt = useMemo(() => {
    return tasks.reduce((sum, task) => sum + Number(task?.value), 0);
  }, [tasks]);
  ...
  const handleDeleteLane = async () => {
    try {
      await deleteLane(laneDetail.id);
      await saveActivityLogNotification(
        accountId,
        `Deleted lane ${laneDetail.name}`
      );
      ...
      console.log(err);
      toast("Oops...", {
        description: "Something went wrong while deleting the lane.",
      });
    }
  };

  const addNewTask = (task: TaskWithTags) => {
  ...
  const handleCreateTask = () => {
    setOpen(
      <CustomModal
        title={projectId ? "Create A Project Task" : "Create A Task"}
        subheading="Tasks items are a great way to keep track of necessary project work."
      >
        <TaskForm
  ...
  return (
    <Draggable
      draggableId={laneDetail.id.toString()}
      index={index}
      key={laneDetail.id}
    >
      {(provided, snapshot) => {
        if (snapshot.isDragging) {
          ...
          //@ts-expect-error - DraggableProps style type doesn't properly expose left property
          const x = provided.draggableProps.style?.left - offset.x;
          //@ts-expect-error - DraggableProps style type doesn't properly expose top property
          const y = provided.draggableProps.style?.top - offset.y;
          //@ts-expect-error - DraggableProps style has a complex type that doesn't allow direct assignment
        ...
        return (
          ...
          >
            <AlertDialog>
              <DropdownMenu>
                <div className="bg-neutral h-full w-[300px] px-2 relative overflow-visible rounded-md flex-shrink-0 ">
                  <div
                    {...provided.dragHandleProps}
                    className=" h-14 backdrop-blur-lg absolute top-0 left-0 right-0 z-10 "
                  >
                    <div className="h-full flex bg-neutral items-center p-2 justify-between cursor-grab">
                      {/*{laneDetail.order}*/}
