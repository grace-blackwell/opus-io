...
import { TaskDetails, TaskWithTags } from "@/lib/types";
...
const KanbanTask = ({ setAllTasks, task, accountId, index }: Props) => {
  const router = useRouter();
  const { setOpen } = useModal();

  const editNewTask = (task: TaskWithTags) => {
    setAllTasks((tasks) => tasks.map((t) => (t.id === task.id ? task : t)));
  };
  ...
  const handleClickEdit = () => {
    setOpen(
      ...
      async () => {
        // Cast the task to Partial<ModalData> to ensure type compatibility
        return { task: task as unknown as TaskDetails };
      }
    );
  };
    ...
    <Draggable draggableId={task.id.toString()} index={index}>
      {(provided, snapshot) => {
        if (snapshot.isDragging) {
          ...
          //@ts-expect-error - DraggableProps style type is read-only in the type definition
              ...
              <DropdownMenu>
                <Card className="my-4 bg-base-100 border-none shadow-md rounded-none">
                  <CardHeader>
                    ...
                    <CardDescription className={"text-xs text-base-content"}>
                      {task.description}
                    </CardDescription>
                          ...
                          <div className="space-y-1">
                            ...
                            <div className="flex items-center pt-2">
                              ...
                              <span className="text-xs text-muted-foreground">
                                Joined{" "}
                                {task.Contact?.createdAt.toLocaleDateString()}
                              </span>
                            </div>
                          </div>
                        </div>
