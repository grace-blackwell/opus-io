  ...
  const handleClickCreateKanban = () => {
    setOpenModal(
      <CustomModal
        ...
        subheading={
          "Kanban boards allow you to group tasks into lanes and track your processes"
        }
      >
      ...
      <div className="flex items-end gap-2">
        <Popover open={open} onOpenChange={setOpen}>
          <PopoverTrigger asChild>
            ...
            >
              {value
                ? kanbans.find((kanban) => kanban.id === value)?.name
            ...
            <Command>
              ...
              <CommandGroup>
                {kanbans.map((kanban) => (
                  <Link
                    key={kanban.id}
                    href={`/account/${accountId}/kanbans/${kanban.id}`}
                  >
                    <CommandItem
                      key={kanban.id}
                      value={kanban.id}
                    ...
                    >
                      <Check
                        className={cn(
                          "mr-2 h-4 w-4",
                          value === kanban.id ? "opacity-100" : "opacity-0"
                        )}
                      />
                      {kanban.name}
                    </CommandItem>
