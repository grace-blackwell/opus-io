...
export async function POST(
  ...
  try {
    ...
    if (contentType.includes('application/xml') || contentType.includes('text/xml') || 
        contentType.includes('application/octet-stream') || contentType.includes('text/plain')) {
      ...
      if (!content.trim().startsWith('<?xml') && !content.trim().startsWith('<mxfile')) {
    ...
    if (contentType.includes('multipart/form-data')) {
      ...
      let xmlFile = formData.get('xmlfile') as File || formData.get('xml') as File
      const pngFile = formData.get('pngfile') as File || formData.get('png') as File
      ...
      if (!xmlContent) {
        ...
        return NextResponse.json({ error: 'No diagram data provided' }, { status: 400 })
      }
      
      // Log the first 100 characters of the XML content for debugging
      console.log('XML content (first 100 chars):', xmlContent.substring(0, 100))
