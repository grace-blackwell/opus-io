...
export async function POST(request: Request) {
  try {
    ...
    console.error("Invoice autofill error:", err);

    // Check for OpenDNS or other network blocking
    if (
      err.message?.includes("block.opendns.com") ||
      (err.status === 403 && err.message?.includes("html"))
    ) {
      return NextResponse.json(
        {
          error:
            "Network restriction detected: Your network administrator has blocked access to the AI service. Please contact your IT department or try using a different network connection.",
        },
        { status: 403 }
      );
    }
    ...
    if (err.message?.includes("API key")) {
      ...
      return NextResponse.json(
        {
          error:
            "OpenAI API quota exceeded. Please check your billing information.",
        },
        { status: 402 }
      );
    } else if (err.message?.includes("invalid_request_error")) {
      return NextResponse.json(
        {
          error:
            "Invalid request to OpenAI API. Please check your configuration.",
        },
        { status: 400 }
      );
      ...
      return NextResponse.json(
        {
          error:
            "Access to the AI service is blocked. This could be due to network restrictions or API key permissions.",
        },
        { status: 403 }
      );
      ...
      return NextResponse.json(
        {
          error: `OpenAI API error (${err.response.status}): ${
    ...
    return NextResponse.json(
      { error: err.message || "Failed to process invoice text" },
      { status: 500 }
    );
  }
}
